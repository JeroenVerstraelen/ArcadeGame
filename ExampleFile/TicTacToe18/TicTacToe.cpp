...
bool legalMoves(const char *stringWithMoves) {
	int movesLength;
	movesLength = strlen(stringWithMoves);
...
void TicTacToePlayer::setMoves(const char *stringWithMoves) {
	...
	_movesLength = strlen(stringWithMoves);
...
void TicTacToe::setMoves(const char *oMoves, const char *xMoves) {
	REQUIRE(this->properlyInitialized(), "TicTacToe wasn't initialized when calling setMoves");
